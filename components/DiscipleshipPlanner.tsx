...
import { UserProfile, PlannedActivity, ActivityType } from '../types';
import { DAYS_OF_WEEK, ACTIVITY_TYPES } from '../constants';
import { getAISuggestionForActivity } from '../services/GeminiService';
...
import { PlusCircleIcon, CalendarDaysIcon, TrashIcon, CheckCircleIcon, PencilSquareIcon, LightBulbIcon } from './common/IconComponents';
...
const DiscipleshipPlanner: React.FC<DiscipleshipPlannerProps> = ({ userProfile }) => {
  ...
  // Example AI Weekly Focus
  const [weeklyFocusTheme, setWeeklyFocusTheme] = useState<string>("Deepening Your Understanding of Grace");
  ...
  return (
    <div className="space-y-6">
      ...
      <Card title="This Week's AI-Powered Focus (Example)" className="bg-brand-accent/10 border-brand-accent" titleClassName="font-display text-xl">
        <div className="flex items-center">
            <LightBulbIcon className="w-8 h-8 text-brand-accent mr-3 flex-shrink-0"/>
            <div>
                <p className="text-brand-text-primary text-base">
                    Based on your goals and recent activity, your suggested focus this week is: 
                    <strong className="text-brand-primary"> "{weeklyFocusTheme}"</strong>.
                </p>
                <p className="text-xs text-brand-text-secondary mt-1">Consider planning activities that explore this theme or related scriptures.</p>
            </div>
        </div>
      </Card>
